...
def losses_to_Z(list_losses):
    lista = []
    df = pd.DataFrame(lista, columns = ['UserID' , 'X1', 'X2', 'X3', 'X4', 'X5', 'X6', 'X7', 'X8', 'X9', 'X10'])
...
def matrices_Zs(Z, G): # retorar uma matriz Z para cada grupo
    ...
    for group in G: # G = {1: [1,3], 2: [2,4,5..40]}
        Z_ = []
        list_users = G[group]
        #for user in range(len(list_users)):
...
users_g1 = ["U{}".format(i + 1) for i in range(2)]  # Usuários de 1 até 2
ls_g1 = ["l{}".format(j + 1) for j in range(len(list_X_est))] # j corresponde a quantidade de matrizes estimadas geradas
users_g2 = ["U{}".format(i + 3) for i in range(38)] # Usuários de 3 até 40
