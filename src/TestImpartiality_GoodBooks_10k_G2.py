...
with open('TestImpartiality_GoodBooks_10k_G2.txt', 'w') as arquivo:

    # reading data from 10000 GoodBooks and 53424 users 
    Data_path = 'Data/GoodBooks-10k'
    n_users=  300
    n_items= 1000
    top_users = True # True: to use users with more ratings; False: otherwise
    top_items = True # True: to use GoodBooks with more ratings; False: otherwise
    ...
    X, users_info, items_info = recsys.read_goodbooks(n_users, n_items, top_users, top_items, data_dir = Data_path) # returns matrix of ratings with n_users rows and n_items columns
    ...
    #X.to_excel('X_GoodBooks.xlsx', index=True)
    #X_est.to_excel('Xest_GoodBooks.xlsx', index=True)
    ...
    arquivo.write(f'Polarization (Rpol): {Rpol:.9f} \n')
    ...
    arquivo.write(f'Individual Loss Variance (Rindv): {Rindv:.9f} \n')
    ...
    arquivo.write(f'Group Loss Variance (Rgrp NR): {RgrpNR:.9f}) ')

    rmse = RMSE(X, omega)
    result = rmse.evaluate(X_est)
    print("RMSE: ", result)
    arquivo.write(f'RMSE: {result:.9f}')
...
##############################################################################################################################
    ...
    for kk in list_k:
        ...
        arquivo.write("\n\n-----------------------------------------------------------------------------\n")
        arquivo.write("-----------------------------------------------------------------------------\n")
        arquivo.write(f'K = {kk} \n')

        list_X_est = algorithmImpartiality.evaluate(X_est, kk) # calculates a list of 3 estimated matrices

        #list_X_est[0].to_excel('X1_GoodBooks.xlsx', index=True)
        ...
        # X_gurobi.to_excel("Xest_GoodBooks_Gurobi.xlsx")
